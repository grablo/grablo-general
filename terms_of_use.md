...
3.  회사가 제공하는 서비스의 내용은 다음과 같습니다.
    ...
    1.  하드웨어 제어를 위한 프로그래밍 제작 플랫폼

    2.  하드웨어 제어 및 모니터링 서비스

    3.  위 가, 나항에 관련된 프로그래밍 관련 커뮤니티

    4.  기타 위 가, 나, 다항 관련 회사가 제공하는 일체의 서비스
...
5.  소프트웨어의 사용 및 성능과 고객지원 서비스에 따른 모든 위험은
    고객이 부담합니다. 회사는 관련 법률이 허용하는 최대한의 범위 내에서
    상품성, 특정 목적에 대한 적합성, 지식재산권 또는 지식재산권의
    비침해성에 대한 묵시적 보증 등을 포함한 명시적 또는 묵시적인 모든
    보증을 배제합니다. 회사는 소프트웨어에 포함된 기능이 고객의
    요구사항을 충족시키거나 소프트웨어의 사용시 일시적인 간섭이나 오류가
    발생하지 않을 것을 보증하지 않으며, 기타 관련 법률이 허용하는
    범위에서 배제 가능한 모든 보증을 배제합니다.

6.  본 계약에 규정된 손해 배상이 그 목적을 달성했는지 여부를 불문하고,
